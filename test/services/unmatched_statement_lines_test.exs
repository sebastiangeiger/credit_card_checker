defmodule CreditCardChecker.UnmatchedStatementLinesTest do
  ...
  test "view_model with a statement line and mis-matching expense" do
    ...
    create_expense(%{amount: 12.34}, payment_method: pm, merchant: merchant, user: user)

    assert UnmatchedStatementLines.view_model(user.id) ==
      [unmatched_but_with_possible_expense: [],
       unmatched_without_possible_expense: [sl]]
  end
  ...
  test "view_model with a statement line and matching expense" do
    ...
    assert UnmatchedStatementLines.view_model(user.id) ==
      [unmatched_but_with_possible_expense: [sl],
       unmatched_without_possible_expense: []]
  end
end
